...
impl ProxyNetwork<Bootstrapped> {
    ...
    async fn discover_and_connect_to_peer(
        ...
        while retry_count < MAX_RETRIES {
            ...
            // Extract the *destination* peer ID from each address. For a
            // circuit-relay multiaddr `/dns4/<hub>/p2p/<HUB>/p2p-circuit/
            // p2p/<DEST>` the address contains TWO P2p protocols — the
            // relay's and the destination's. We want the last one
            // (the actual node behind the circuit). For a direct
            // address (no `/p2p-circuit/`) the last P2p IS the
            // destination.
            //
            // This set is what the wait_for_with_timeout predicate
            // filters on, so we only treat ConnectionEstablished
            // events for these peers as "discovery succeeded" — fixes
            // two bugs:
            //   1. The predicate previously matched the relay's
            //      ConnectionEstablished event (the first to fire on
            //      a circuit dial) and stored the HUB as the
            //      destination peer.
            //   2. Late-arriving connections from the *previous*
            //      server's dial set leaked into the next server's
            //      wait — making a port configured for NZ store an AT
            //      peer that another server had dialed.
            let our_destinations: std::collections::HashSet<PeerId> =
            ...
            let wait_result = tokio::select! {
                r = self.0.swarm.wait_for_with_timeout(
                    move |_, event| {
    ...
    async fn handle_proxy_events(
        ...
        match message {
            ...
            SocksStreamMessage::RequestNewPeer {
                ...
                match self.discover_and_connect_to_peer(server_config, shutdown).await {
                    Ok(p) => {
            ...
            SocksStreamMessage::PeerDisconnected {
                ...
                match self
                    .discover_and_connect_to_peer(server_config, shutdown)
                    .await
                {
                    Ok(new_peer) => {
