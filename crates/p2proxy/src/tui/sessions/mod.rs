...
impl Ui {
    ...
    fn render_sessions_table(&self, frame: &mut Frame<'_>, area: Rect) {
        ...
        let rows: Vec<Row> = if self.state.sessions.is_empty() {
            // Empty table looks identical to "p2proxy is broken" — give
            // the operator a one-line nudge so they know the binary is
            // up and just waiting for a SOCKS client to connect.
            vec![Row::new(vec![
                Cell::from("—"),
                Cell::from("—"),
                Cell::from("Waiting for SOCKS client to open a proxy session…")
                    .style(Style::default().fg(SECONDARY)),
                Cell::from("Idle").style(Style::default().fg(SECONDARY)),
            ])
            .height(1)]
        } else {
            self.state
