...
use p2p_bandwidth_protocol::{Session, SessionInit, SessionTransfer, TargetAddr, TCP_PROXY_PROTOCOL};
...
pub async fn run_socks_proxy(
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let session_init_bytes = postbag::to_stdvec(&request).unwrap();
                ...
                select! {
                    result = socket_read.read(&mut socket_buf) => match result {
                        ...
                        Ok(n) => {
                            ...
                            if let Ok(transfer_bytes) = postbag::to_stdvec(&transfer) {
                    ...
                    result = read_length_prefixed(&mut stream_read) => match result {
                        Ok(data) => {
                            ...
                            match postbag::from_bytes::<Session>(&data) {
